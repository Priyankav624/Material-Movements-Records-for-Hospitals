...
// import { 
//   Table, Button, Select, DatePicker, Space, Card, Statistic, Alert,
...
//     } catch (error) {
//       console.error('Error fetching movement logs:', error);
//       message.error('Failed to fetch movement logs');
//     } finally {
//       setLoading(false);
//     }
//   };
...
//         params.endDate = filters.dateRange[1].format('YYYY-MM-DD');
//       }
//       if (filters.action) params.actionType = filters.action;
...
//       saveAs(blob, `movement-report-${new Date().toISOString()}.pdf`);
...
//           <div>{text || 'N/A'}</div>
//           {record.batchNumber && (
//             <small className="text-muted">Batch: {record.batchNumber}</small>
//           )}
//         </div>
//       )
//     },
...
const handleReturn = async (log) => {
  ...
  // In your MovementLogs.jsx component
const fetchMovementLogs = async () => {
  setLoading(true);
  try {
    const token = localStorage.getItem('token');
